...
public class MaterialTabHost extends Toolbar {
	...
	public MaterialTabHost(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// get attributes
		if(attrs != null) {
			TypedArray a = context.getTheme().obtainStyledAttributes(attrs,R.styleable.AppTheme_Base, 0, 0);
			
			try {
				hasIcons = a.getBoolean(R.styleable.AppTheme_Base_hasIcons, false);
			} finally {
				a.recycle();
			}
		}
